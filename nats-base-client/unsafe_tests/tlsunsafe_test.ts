...
Deno.test("tls-unsafe - handshake first", async () => {
  ...
  const config = {
    ...
    tls: {
      ...
      cert_file: resolve(
        join(cwd, "./nats-base-client/tests/certs/localhost.crt"),
      ),
      key_file: resolve(
        join(cwd, "./nats-base-client/tests/certs/localhost.key"),
      ),
      ca_file: resolve(join(cwd, "./nats-base-client/tests/certs/RootCA.crt")),
    },
  };
